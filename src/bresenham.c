...
/*   Updated: 2023/11/09 18:42:46 by bhildebr         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
void	bresenham(mlx_image_t *img, struct s_point p0, struct s_point p1)
{
	...
	num_steps = (t_i32)round(fmax(fabs(delta.x), fabs(delta.y)));
	if (num_steps == 0)
	{
		put_valid_pixel(img, p0.position.x, p0.position.y, (t_u32)
			(t_u32)p0.color.r << 24 | (t_u32)p0.color.g << 16 | (t_u32)p0.color.b << 8 | (t_u32)p0.color.a);
		return ;
	}
